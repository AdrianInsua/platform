...
describe('NgRedux Observable Store', () => {
    ...
    it('should throw when the store is configured twice', () => {
        ...
        // it a second time.
        expect(ngRedux.configureStore.bind(ngRedux, rootReducer, defaultState)).toThrowError(Error);
    });
    ...
    it('should accept a custom compare function', () => {
        ...
        const spy = jasmine.createSpy('spy').and.callFake((data: Record) => (fooData = data));
        const cmp = (a: Record, b: Record) => a.data === b.data;

        ngRedux.select(state => ({ data: `${state.foo}-${state.baz}` }), cmp).subscribe(spy);
    ...
    it('should only call provided select function if state changed', () => {
        const selectSpy = jasmine.createSpy('selectSpy').and.callFake((state: AppState) => state.foo);
    ...
    it('should have select decorators work before store is configured', done => {
        ...
        someService.foo$.pipe(combineLatest(someService.bar$, someService.baz$)).subscribe(([ foo, bar, baz ]) => {
...
describe('Chained actions in subscriptions', () => {
    ...
    describe('dispatching an action in a keyword$ before length$ happens', () => {
        it('length sub should be called twice', () => {
            ...
            const lengthSpy = jasmine.createSpy('lengthSpy').and.callFake((n: number) => (length = n));
            let lenSub;
            let keywordSub;
