        ...
        for (const value of iterable) {
            const viewRef =
                this.viewContainerRef.length > index
                    ? (this.viewContainerRef.get(index) as EmbeddedViewRef<ConnectArrayTemplate>)
                    : null;

            if (viewRef == null) {
                const embeddedViewRef = this.viewContainerRef.createEmbeddedView<ConnectArrayTemplate>(
                    this.templateRef,
